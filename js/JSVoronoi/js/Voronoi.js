...
Voronoi.prototype.draw = function(data, options)
{
	...
	if (this.voronoi == undefined)
		this.voronoi = new JSVoronoi(xPos, yPos, w, h, colourGradient, this.containerElement, fillPolygons, tooltips);
 ...
 * *********************************
 */
JSVoronoi = function(x, y, width, height, colourGradient, targetElement, fillRegions, tooltips)
{
	this.targetDiv;
	var id = allocateId();
	var canvas;
	...
	function render(data)
	{
		...
		do 
	    {
			...
			var v = new Vertex(xCoord, yCoord, 1);
			generators[i] = v;
	    }
	    while (i-- > 0)
	...
	this.addNewPoint = function(p)
	{
		...
		var transformedP = new Vertex(xPos, yPos, 1);
	...
	*/
	function getClosestLeafNode(p, node)
	{
		...
		if (node.nodeType != QuadTreeNode.LEAF_NODE)
		{
			for (var j = 0; j < 4; j++)
			{
				...
				if (testNode.getGeneratorCount() > 0)
				{
					...
					var scaledNodePoint = new Vertex(calcPosX(tn.x), calcPosY(tn.y), 1);
	...
	*/
	function createSeedGraph()
	{
		...
		p1 = new Vertex(0.5, 3.0*Math.sqrt(2.0)/2.0 + 0.5, 1);
		
		p2 = new Vertex(-3.0*Math.sqrt(6.0)/4.0 + 0.5, -3.0*Math.sqrt(2.0)/4.0 + 0.5, 1);
		
		p3 = new Vertex(3.0*Math.sqrt(6.0)/4.0 + 0.5, -3.0*Math.sqrt(2.0)/4.0 + 0.5, 1);
	...
	*/
	 ...
	 newPolygon)
	{
		...
		var mid = new Vertex((x1 + x2) / 2.0, (y1 + y2) / 2.0, 0); // Mid point of line
		var bi = new Vertex((y1 - y2), -(x1 - x2), 0); // Direction of perpendicular line
	...
	*/
	 ...
	 newPolygon)
	{
		...
		var v = new Vertex(vTemp.x, vTemp.y, 1);
		...
		var v2 = new Vertex(vTemp.x, vTemp.y, 1);
	...
	*/
	...
	function displayTooltip(e)
	{
		...
		var position = new Vertex(mousePosX, mousePosY, 1);
		var rootNode = qTree.getRootNode();
...
QuaternaryTree = function(generators, p1)
{
	...
	*/
	function createTree()
	{
		...
		rootNode.nodeType = QuadTreeNode.ROOT_NODE;
	...
	*/
	function buildTree(kIn, n, rStart, rEnd, cStart, cEnd)
	{
		...
		else
		{
			...
			n.nodeType = QuadTreeNode.LEAF_NODE;
	...
	*/
	...
	function populateParentNodes(child, g)
	{
		...
		if ((parent.nodeType != QuadTreeNode.ROOT_NODE) && (parent.getGeneratorCount() == 0))
		{
...
QuadTreeNode.ROOT_NODE = 0;
QuadTreeNode.INTERMEDDIATE_NODE = 1;
QuadTreeNode.LEAF_NODE = 2;